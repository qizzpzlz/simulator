...
double Node:: execute(double current_time)
{
	Task& task = *current_task_ptr_;
	...
	if (current_time < current_time_ + task.leftTime) {
		...
		task.execution_time += current_time - current_time_;
		...
		task.execution_time += task.leftTime;
		task.leftTime = 0;
		task.finished_time = current_time_;
